...
fn netcdf_to_parquet(config: &ConvertConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Extract ar data
    let df_ar_data = collect_netcdf_cora2_data(src_file)?;
...
fn collect_netcdf_cora2_data(
    ...
    // Meta info
    let time: Vec<f64> = common::get_coordinate_value(&file, "TIME".to_string(), n_profs, n_levels, f64::NAN)?;
    ...
    let time_qc: Vec<i8> = common::get_coordinate_value(&file, "TIME_QC".to_string(), n_profs, n_levels, i8::MIN)?;
    let position_qc: Vec<i8> = common::get_coordinate_value(&file, "POSITION_QC".to_string(), n_profs, n_levels, i8::MIN)?;
