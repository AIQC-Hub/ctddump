...
fn netcdf_to_parquet(config: &ConvertConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Extract ar data
    let df_ar_data = collect_netcdf_nrt_ar_data(src_file)?;
...
fn collect_netcdf_nrt_ar_data(
    ...
    let pres_conv: Vec<i8> = vec![0_i8; pres.len()];

    // Convert PRES to DEPH
    let deph: Vec<f32> = vec![pres_fill_value; pres.len()];
    let deph_qc: Vec<i8> = vec![i8::MIN; pres_qc.len()];
    let (converted_deph, converted_deph_qc, deph_conv) = common::convert_pressure_to_depth(deph.clone(), deph_qc.clone(), pres.clone(), pres_qc.clone(), pres_fill_value, latitude.clone());
