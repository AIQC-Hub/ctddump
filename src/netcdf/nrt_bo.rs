...
fn collect_netcdf_nrt_bo_data(
    ...
    let prc_lon_fill_value: f32 = longitude[0];
    let precise_longitude: Vec<f32> = common::get_coordinate_value(&file, "PRECISE_LONGITUDE".to_string(), time_len, obs_len, prc_lon_fill_value)?;
    let latitude: Vec<f32> = common::get_coordinate_value(&file, "LATITUDE".to_string(), time_len, obs_len, f32::NAN)?;
    let prc_lat_fill_value: f32 = latitude[0];
    let precise_latitude: Vec<f32> = common::get_coordinate_value(&file, "PRECISE_LATITUDE".to_string(), time_len, obs_len, prc_lat_fill_value)?;
    ...
    let (converted_pres, converted_pres_qc, pres_conv) = common::convert_depth_to_pressure(pres.clone(), pres_qc.clone(), deph.clone(), deph_qc.clone(), pres_fill_value, precise_latitude.clone());

    // Convert PRES to DEPH when DEPH is NA
    let (converted_deph, converted_deph_qc, deph_conv) = common::convert_pressure_to_depth(deph.clone(), deph_qc.clone(), pres.clone(), pres_qc.clone(), deph_fill_value, precise_latitude.clone());
    ...
    let df = DataFrame::new(vec![
        ...
        Series::new("longitude".into(), precise_longitude),
        Series::new("latitude".into(), precise_latitude),
