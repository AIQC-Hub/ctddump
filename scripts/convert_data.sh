...
#   --time-log FILE   Write the --time measurements to FILE (implies --time),
#                     so the screen keeps only the normal progress output.
...
convert() {  # <format> <src_dir> <out_dir>
  mkdir -p "$3"
  log "convert $1: $2 -> $3"
  measure "convert $1" ctddump batch convert "$1" --threads "$THREADS" --output "$3" "$2"
}

merge() {  # <src_dir> <out_file>
  mkdir -p "$(dirname "$2")"
  log "merge: $1 -> $2"
  measure "merge ${2##*/}" ctddump concat convert --threads "$THREADS" "$1" "$2"
}

header_nrt() {  # <pattern> <src_dir> <out_dir>
  mkdir -p "$3"
  log "header nrt ($1): $2 -> $3"
  measure "header ${3##*/}" ctddump batch header nrt --threads "$THREADS" --pattern "$1" --output "$3" "$2"
}

header_cora() {  # <src_dir> <out_dir>
  mkdir -p "$2"
  log "header cora: $1 -> $2"
  measure "header ${2##*/}" ctddump batch header cora --threads "$THREADS" --output "$2" "$1"
}

merge_hdr() {  # <src_dir> <out_file>
  mkdir -p "$(dirname "$2")"
  log "merge header: $1 -> $2"
  measure "merge header ${2##*/}" ctddump concat header "$1" "$2"
}
...
report_parquet() {  # <src_file> <out_tsv> -- platform-level summary of a merged Parquet
  ...
  measure "report ${2##*/}" ctddump report parquet --level platform "$1" "$2"
}
...
report_yaml() {  # <src_file> <out_tsv> -- summary of a merged header YAML
  ...
  mkdir -p "$(dirname "$2")"
  log "report yaml: $1 -> $2"
  measure "report ${2##*/}" ctddump report yaml "$1" "$2"
}
